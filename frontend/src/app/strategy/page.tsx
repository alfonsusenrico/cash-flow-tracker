...
interface Rule { rule_id: string; name: string; trigger: string; mode: string; target_bucket_id: string | null; target_bucket_name: string | null; value: number; cap: number | null; floor: number | null; priority: number; is_active: boolean; notes: string | null; }
interface Preview { income: number; total_allocated: number; remaining: number; allocations: { rule_id: string; rule_name: string; target_bucket_name: string | null; mode: string; amount: number }[]; }
...
export default function StrategyPage() {
  ...
  return (
    <div className="p-5">
      <p className="text-xs text-[var(--muted)] mb-4">Set the rules for how your income is distributed across buckets.</p>
      <div className="grid grid-cols-2 gap-5">
        ...
        <Card padding="sm">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-[var(--muted)] -mt-2">Rules are applied from top to bottom by priority.</p>
            </div>
                  ...
                  <td className="py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[var(--muted)]">{r.target_bucket_name ?? "—"}</p>
                      </div>
                    </div>
                  </td>
          ...
          <div className="mt-3 p-3 rounded-lg bg-[var(--bg)] text-xs text-[var(--muted)] flex items-start gap-2">
            ...
            <div>
              ...
              <p>Fixed Amount rules are deducted first. Percentage rules are calculated from the remaining income.</p>
            </div>
          </div>
        </Card>
        ...
        <Card padding="md">
          ...
          <p className="text-xs text-[var(--muted)] mb-2">See how your income will be distributed based on the rules.</p>
          ...
          <Button variant="primary" className="w-full mb-4" onClick={() => previewMut.mutate()} disabled={previewMut.isPending}>
          ...
          {preview && (
            <>
                ...
                <tbody className="divide-y divide-[var(--border)]">
                  {preview.allocations.map((a, i) => (
                    <tr key={a.rule_id}>
                      ...
                      <td className="py-2 font-medium">{a.target_bucket_name ?? a.rule_name}</td>
              ...
              <Button variant="primary" className="w-full mt-3">✓ Save Strategy Rules</Button>
            </>
          )}
        </Card>
      </div>
