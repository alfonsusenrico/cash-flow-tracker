...
def current_period_value(period_type: str) -> str:
    now = now_utc()
...
def parse_period_parts(period_type: str, period_value: str | None) -> tuple[int, int | None]:
    if period_type == "month":
        match = re.match(r"^(\d{4})-(\d{2})$", period_value or "")
        if not match:
        ...
        return year, month
    if period_type == "quarter":
        match = re.match(r"^(\d{4})-Q([1-4])$", period_value or "")
        if not match:
        ...
        return year, quarter
    if period_type == "year":
        match = re.match(r"^(\d{4})$", period_value or "")
        if not match:
...
def shift_period_value(period_type: str, period_value: str, delta: int) -> str:
    year, sub = parse_period_parts(period_type, period_value)
    if period_type == "month":
        index = year * 12 + (sub or 1) - 1 + delta
        next_year = index // 12
        next_month = index % 12 + 1
        return f"{next_year}-{next_month:02d}"
    if period_type == "quarter":
        index = year * 4 + (sub or 1) - 1 + delta
        next_year = index // 4
        next_quarter = index % 4 + 1
        return f"{next_year}-Q{next_quarter}"
    return str(year + delta)


def build_period_info(period_type: str, period_value: str | None) -> dict[str, Any]:
    if period_type not in ("month", "quarter", "year"):
        raise HTTPException(status_code=400, detail="Invalid period type")
    value = period_value or current_period_value(period_type)
    try:
        year, sub = parse_period_parts(period_type, value)
    except ValueError:
        year, sub = parse_period_parts(period_type, current_period_value(period_type))
        value = current_period_value(period_type)
...
def months_in_range(from_dt: datetime, to_dt: datetime) -> list[str]:
    year = from_dt.year
    month = from_dt.month
    end_year = to_dt.year
    end_month = to_dt.month
    months = []
...
def fetch_period_totals(cur, username: str, from_dt: datetime, to_dt: datetime) -> dict[str, int]:
    cur.execute(
        """
...
def fetch_top_spending(cur, username: str, from_dt: datetime, to_dt: datetime, total_out: int) -> list[dict[str, Any]]:
    cur.execute(
        """
    ...
    for r in cur.fetchall():
        amount = int(r.get("total_out") or 0)
        pct = amount / total_out if total_out > 0 else 0
        rows.append(
            {
                "account_id": r["account_id"],
                "account_name": r["account_name"],
                "total_out": amount,
                "pct_of_total_out": pct,
            }
        )
    return rows


def fetch_budget_utilization(
    cur,
    username: str,
    months: list[str],
    from_dt: datetime,
    ...
    cur.execute(
        ...
        SELECT b.account_id::text AS account_id,
               a.account_name,
               COALESCE(SUM(b.amount), 0) AS limit_amount
        FROM budgets b
        JOIN accounts a ON a.account_id=b.account_id
        WHERE b.username=%s
          AND b.month = ANY(%s::text[])
        GROUP BY b.account_id, a.account_name
        """,
        (username, months),
    )
    budgets = {r["account_id"]: r for r in cur.fetchall()}

    cur.execute(
        """
        SELECT t.account_id::text AS account_id,
               COALESCE(SUM(CASE WHEN t.transaction_type='credit' THEN t.amount ELSE 0 END), 0) AS used
        FROM transactions t
        JOIN accounts a ON a.account_id=t.account_id
        WHERE a.username=%s
          AND a.parent_account_id IS NOT NULL
    ...
    used_map = {r["account_id"]: int(r.get("used") or 0) for r in cur.fetchall()}

    results = []
    for account_id, row in budgets.items():
        limit_amount = int(row.get("limit_amount") or 0)
        used = used_map.get(account_id, 0)
        remaining = max(0, limit_amount - used)
        if limit_amount <= 0:
            status = "critical"
            pct = 1.0
        else:
            pct = used / limit_amount
            if pct >= 1:
                status = "critical"
            elif pct >= 0.8:
                status = "warn"
    ...
    status_rank = {"critical": 0, "warn": 1, "ok": 2}
    results.sort(key=lambda r: (status_rank.get(r["status"], 3), -r["pct"], r["account_name"]))
    return results
...
def insights(
    ...
    username = require_user(req)
    period_type = type or "month"
    period_info = build_period_info(period_type, period)
    cache_key = f"{username}:insights:{period_info['type']}:{period_info['value']}"
    cached = cache_get(cache_key)
    if cached:
        return cached

    with db() as conn, conn.cursor() as cur:
        totals = fetch_period_totals(cur, username, period_info["from_dt"], period_info["to_dt"])
        prev_value = shift_period_value(period_type, period_info["value"], -1)
        prev_info = build_period_info(period_type, prev_value)
        prev_totals = fetch_period_totals(cur, username, prev_info["from_dt"], prev_info["to_dt"])

        top_spending = fetch_top_spending(
            cur, username, period_info["from_dt"], period_info["to_dt"], totals["total_out"]
        )

        months = months_in_range(period_info["from_dt"], period_info["to_dt"])
        budgets = fetch_budget_utilization(cur, username, months, period_info["from_dt"], period_info["to_dt"])

        spark_values = [shift_period_value(period_type, period_info["value"], -i) for i in range(5, -1, -1)]
        spark_labels: list[str] = []
        spark_in: list[int] = []
        spark_out: list[int] = []
        spark_net: list[int] = []
        for value in spark_values:
            info = build_period_info(period_type, value)
            spark_totals = fetch_period_totals(cur, username, info["from_dt"], info["to_dt"])
            spark_labels.append(info["label"])
            spark_in.append(spark_totals["total_in"])
            spark_out.append(spark_totals["total_out"])
            spark_net.append(spark_totals["net"])
    ...
    payload = {
        "period": {
            ...
            "from": period_info["from"],
            "to": period_info["to"],
        },
        "previous": {
            "type": prev_info["type"],
            "value": prev_info["value"],
            "label": prev_info["label"],
            "from": prev_info["from"],
            "to": prev_info["to"],
        },
        "totals": totals,
    ...
    cache_set(cache_key, payload, INSIGHTS_CACHE_TTL)
    return payload
